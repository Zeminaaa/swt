class Pole:
    def __init__(self,radek,sloupec,hodnota):
        self.radek=radek
        self.sloupec=sloupec
        self.hodnota=hodnota

    def __str__(self):
        if self.hodnota is not None:
            return self.hodnota
        else:
            return(".")
...
class Sachovnice:
    def __init__(self,velikost=velikost):
        ...
        self.policka=[[Pole(radek, sloupec, None) for sloupec in range(velikost)] for radek in range(velikost)]
    
    def tah(self, radek, sloupec, hodnota):
        radek -= 1
        sloupec -= 1
        if self.policka[radek][sloupec].hodnota is None:
            self.policka[radek][sloupec].hodnota = hodnota
        else:
        ...
        def kontroluj_smer(zmena_radek, zmena_sloupec):
            ...
            while True:
                ...
                if 0 <= novy_radek < self.velikost and 0 <= novy_sloupec < self.velikost:
                    if self.policka[novy_radek][novy_sloupec].hodnota == hodnota:
                        rada += 1
                        posun += 1
                    else:
                        break
